...
class CarHistory {
    constructor(name, ano_modelo, valueChanged, baseYear, percentageChanged, preco) {
        this.name = name;
...
let HomeComponent = class HomeComponent {
    constructor(formBuilder, brandService, carService) {
...
let AuthenticationService = AuthenticationService_1 = class AuthenticationService {
    ...
    login(email, password) {
        return this.http.post('/api/login', { email, password }, { observe: 'response' })
            .pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["map"])(response => {
            const token = response.headers.get(AuthenticationService_1.authorizationHeader);
            const authentication = new _model__WEBPACK_IMPORTED_MODULE_5__["Authentication"](token);
...
let CarService = class CarService {
    ...
    findAllByBrandId(brandId) {
        return this.http.get(`api/car/all/${brandId}`);
    }
    findHistoryByIdAndModels(carId, brandId) {
        const params = new _angular_common_http__WEBPACK_IMPORTED_MODULE_2__["HttpParams"]().set('brandId', brandId);
        return this.http.get(`api/car/${carId}`, { params });
    }
};
